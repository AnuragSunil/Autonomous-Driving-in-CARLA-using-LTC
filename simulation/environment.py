    ...
    def step(self, action_idx):
        try:
            ...
            if self.continous_action_space:
                steer = float(action_idx[0,0].item())
                steer = max(min(steer, 1.0), -1.0)
                throttle = float((action_idx[0,1].item() + 1.0)/2)
            ...
            if len(self.collision_history) != 0:
                ...
                reward = -10
                print("Deviation from center")
            elif self.episode_start_time + 10 < time.time() and self.velocity < 1.0:
                reward = -10
                done = True
                print("episode_start time< time and velocit <1")
            elif self.velocity > self.max_speed:
                reward = -10
                done = True
                print("above max speed:",self.velocity)
